...
var TrustStrategy = {
  ...
  TRUST_SERVER_CLIENT_CERTIFICATES: function TRUST_SERVER_CLIENT_CERTIFICATES(config, onSuccess, onFailure) {
    ...
    var tlsOpts = {
      ...
      key: _fs2.default.readFileSync(config.key),
      cert: _fs2.default.readFileSync(config.cert),
  ...
  TRUST_ON_FIRST_USE: function TRUST_ON_FIRST_USE(config, onSuccess, onFailure) {
    ...
    var socket = _tls2.default.connect(config.url.port, config.url.host, tlsOpts, function () {
      ...
      if (!serverCert.raw) {
        ...
        onFailure((0, _error2.newError)("You are using a version of NodeJS that does not " + "support trust-on-first use encryption. You can either upgrade NodeJS to " + "a newer version, use `trust:TRUST_CUSTOM_CA_SIGNED_CERTIFICATES` in your driver " + "config instead, or disable encryption using `encrypted:\"" + _util2.ENCRYPTION_OFF + "\"`."));
        return;
      }
      ...
      loadFingerprint(serverId, knownHostsPath, function (knownFingerprint) {
        if (knownFingerprint === serverFingerprint) {
          ...
          onFailure((0, _error2.newError)("Database encryption certificate has changed, and no longer " + "matches the certificate stored for " + serverId + " in `" + knownHostsPath + "`. As a security precaution, this driver will not automatically trust the new " + "certificate, because doing so would allow an attacker to pretend to be the Neo4j " + "instance we want to connect to. The certificate provided by the server looks like: " + serverCert + ". If you trust that this certificate is valid, simply remove the line " + "starting with " + serverId + " in `" + knownHostsPath + "`, and the driver will " + "update the file with the new certificate. You can configure which file the driver " + "should use to store this information by setting `knownHosts` to another path in " + "your driver configuration - and you can disable encryption there as well using " + "`encrypted:\"" + _util2.ENCRYPTION_OFF + "\"`."));
        }
      });
    });
    socket.on('error', onFailure);
...
function connect(config, onSuccess) {
  ...
  if (config.encrypted === false || config.encrypted === _util2.ENCRYPTION_OFF) {
    ...
    value: function _setupConnectionTimeout(config, socket) {
      ...
      if (timeout) {
        ...
        socket.on('timeout', function () {
          ...
          socket.setTimeout(0);
          socket.destroy((0, _error2.newError)('Failed to establish connection in ' + timeout + 'ms', config.connectionErrorCode));
        });
